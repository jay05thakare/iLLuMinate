...
const FacilityModal = ({ isOpen, onClose, onSave, facility = null, mode = 'add', isSubmitting = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await onSave(facilityData);
      
      if (result && result.success) {
        showSuccess(
          `Facility ${mode === 'add' ? 'created' : 'updated'} successfully`,
          { title: mode === 'add' ? 'Facility Created' : 'Facility Updated' }
        );
        onClose();
      } else {
        throw new Error(result?.message || `Failed to ${mode} facility`);
      }
    } catch (error) {
      handleError(error, { 
        context: `${mode === 'add' ? 'Creating' : 'Updating'} facility`,
        formData 
      });
    } finally {
